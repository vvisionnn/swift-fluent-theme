	...
	func fitIntoSuperview(
		...
		if usingConstraints {
			...
			if autoWidth {
				...
				widthAnchor.constraint(equalTo: superview.widthAnchor, constant: -(margins.left + margins.right))
					.isActive = true
			}
			...
			if autoHeight {
				...
				heightAnchor.constraint(equalTo: superview.heightAnchor, constant: -(margins.top + margins.bottom))
					.isActive = true
			}
		} else {
...
extension UIView {
	...
	func contain(
		...
		respectingSafeAreaInsets respectsSafeAreaInsets: Bool = false
	) {
		NSLayoutConstraint.contain(
			view: view,
			in: self,
			withInsets: insets,
			respectingSafeAreaInsets: respectsSafeAreaInsets
		)
	}
}
