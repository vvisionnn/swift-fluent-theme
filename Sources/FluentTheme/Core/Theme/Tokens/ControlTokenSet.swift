...
public class ControlTokenSet<T: TokenSetKey>: ObservableObject {
	...
	public subscript(token: T) -> ControlTokenValue {
		get {
			if let value = overrideValue(forToken: token) {
				value
			} else if let value = defaults?(token, fluentTheme) {
				value
			} else {
				preconditionFailure()
			}
	...
	func registerOnUpdate(for control: UIView, onUpdate: @escaping (() -> Void)) {
		...
		notificationObserver = NotificationCenter.default.addObserver(
			forName: .didChangeTheme,
			object: nil,
			queue: nil
		) { [weak self, weak control] notification in
	...
	var fluentTheme: FluentTheme = .shared {
		didSet {
...
public enum ControlTokenValue {
	...
	public var uiColor: UIColor {
		if case let .uiColor(uiColor) = self {
			return uiColor()
		} else if case let .color(color) = self {
	...
	public var color: Color {
		if case let .color(color) = self {
			return color()
		} else if case let .uiColor(uiColor) = self {
	...
	public var uiFont: UIFont {
		if case let .uiFont(uiFont) = self {
			return uiFont()
		} else {
	...
	public var shadowInfo: ShadowInfo {
		if case let .shadowInfo(shadowInfo) = self {
			return shadowInfo()
		} else {
			assertionFailure("Cannot convert token to ShadowInfo: \(self)")
			return ShadowInfo(
				keyColor: fallbackUIColor,
				keyBlur: 10.0,
				xKey: 10.0,
				yKey: 10.0,
...
extension ControlTokenValue: CustomStringConvertible {
	...
	public var description: String {
		...
		case let .float(float):
			"ControlTokenValue.float (\(float())"
		case let .uiColor(uiColor):
			"ControlTokenValue.uiColor (\(uiColor())"
		case let .color(color):
			"ControlTokenValue.color (\(color())"
		case let .uiFont(uiFont):
			"ControlTokenValue.uiFont (\(uiFont())"
		case let .shadowInfo(shadowInfo):
			"ControlTokenValue.shadowInfo (\(shadowInfo())"
		}
	}
}
#endif // DEBUG
