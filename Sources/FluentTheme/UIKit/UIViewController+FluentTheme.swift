...
extension UIViewController {
	/// The `FluentTheme` in effect for this view controller, resolved from its trait collection.
	///
	...
	/// Applies a `FluentTheme` to this view controller and its entire subtree (its view and child view
	/// controllers), via trait overrides.
	///
	...
	/// Registers a handler invoked whenever the effective `FluentTheme` changes for this view controller.
	///
	...
	///   retain it — the observation is removed automatically when this view controller is deallocated.
	@discardableResult
	public func onFluentThemeChange(
		...
		let registration = registerForTraitChanges([FluentThemeTrait.self]) { (controller: Self, _: UITraitCollection) in
			handler(controller.fluentTheme)
		}
