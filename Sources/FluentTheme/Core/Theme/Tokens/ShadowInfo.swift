...
extension ShadowInfo {
	...
	public func applyShadow(to view: UIView, parentController: UIViewController? = nil) {
		guard var shadowable = (view as? (any Shadowable)) ?? (view.superview as? (any Shadowable)) ??
			(parentController as? (any Shadowable)) else {
	...
	public func initializeShadowLayer(view: UIView, isAmbientShadow: Bool = false) -> CALayer {
		let layer = CALayer()
		...
		layer.shadowOffset = CGSize(
			width: isAmbientShadow ? xAmbient : xKey,
